...
module.exports = function(spec) {
  ...
  obj.move = function(args, callback) {
    ...
    if (fs.statSync(args.sourcePath).isDirectory()) {
      args.sourcePath = path.join(args.sourcePath, '*');
    }

    try {
      shell.mv('-f', args.sourcePath, args.targetPath);
    } catch (err) {
      if (err) return callback(err);
    }
  ...
  obj.copy = function(args, callback) {
    ...
    try {
      shell.cp('-rf', args.sourcePath, args.targetPath);
    } catch (err) {
      if (err) return callback(err);
    }
