...
module.exports = function(spec) {
  ...
  spec = spec || {};
  spec.ssh_tty = spec.ssh_tty || true;
  spec.ssh_port = spec.ssh_port || 22;
  spec.share_connection = spec.share_connection || true;
  ...
  obj.exec = function(args, callback) {
    if (args.options) {
      ...
      args.path = args.path || args.options.cwd;
    }

    var stdout = '';
    var stderr = '';

    var child = spawn('ssh', sshOptions.concat([ c.cmd(args) ]), args.options);
